...
export function usePaperProtocol() {
  ...
  const getDeposit = useCallback(
    async (tokenId: number): Promise<Deposit | null> => {
      try {
  ...
  // Write functions
  const depositEther = useCallback(
    async (amount: string, priceTarget: string) => {
  ...
  const depositToken = useCallback(
    async (tokenAddress: string, amount: string, priceTarget: string) => {
      const walletClient = await createWalletClientAsync();
